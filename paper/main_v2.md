...
**[Figure 1: Pipeline block diagram. Five boxes connected by arrows: (i) Grayscale 256x256 input, (ii) PCA Residual (remove top 32 components), (iii) Three Descriptors (FFT 64-bin + LBP 59-bin + Noise 64-bin), (iv) Concatenation (187-dim vector), (v) StandardScaler + Classifier. The diagram should show the flow left-to-right with the dimensionality annotated at each stage.]**
...
**[Figure 2: PCA-residual visualization. Two rows of images. Top row: four sample face images (two real from FFHQ, two fake from StyleGAN), original grayscale. Bottom row: the corresponding PCA residuals after removing 32 components. The residual images should appear as faint texture patterns, with the dominant face structure removed. Caption should note that the residuals are contrast-enhanced for visibility.]**
...
**Dataset.** 40,000 face images, balanced: 20,000 real images from FFHQ (Flickr-Faces-HQ, introduced by Karras, Laine, and Aila [16]) and 20,000 AI-generated images from the StyleGAN-based "1 Million Fake Faces" collection on Kaggle [17]. Every image is converted to grayscale and resized to 256 x 256 pixels.

**Split.** The data is divided 80/10/10 into training, validation, and test sets using stratified sampling with random seed 42. This gives 32,000 training images, 4,000 validation images, and 4,000 test images. Each split is balanced between the two classes.
...
**Reproducibility.** The pipeline was executed end to end and independently rerun once on the same data and seed. The two runs agreed within ordinary floating-point variation. The numbers reported below come from that verified pipeline.
...
Table 1 shows the results. Every classifier beats the trivial 50 percent baseline by a wide margin.
...
**[Figure 3: ROC curves for all five classifiers on the test set. Five curves plotted on the same axes (FPR on x-axis, TPR on y-axis), with a diagonal reference line for random guessing. XGBoost and LightGBM should be nearly overlapping and closest to the top-left corner. RandomForest should be the lowest. Legend shows classifier name and AUC value.]**

### 6.2 Which Classifier Family Wins
...
**[Figure 4: Bar chart comparing test accuracy of all five classifiers. Five vertical bars, one per classifier, with the y-axis showing accuracy from 0.80 to 0.90. A horizontal dashed line at 0.50 marks the random baseline. XGBoost and LightGBM bars should be tallest, nearly equal. RandomForest bar should be visibly shorter.]**

### 6.3 The Linear Baseline Tells a Story

The thresholded linear regression is the most informative single result in Table 1. It reaches 0.8552 test accuracy and 0.9309 AUC. That is within about one and a half percentage points of the best boosted tree. A purely linear decision boundary over 187 histogram bins already separates the two classes well.
...
### 6.4 Caveats and Threats to Validity

Several caveats bound what these numbers support.

The PCA truncation level $N = 32$ was inherited from [1] and never swept. We do not know whether a different value would change accuracy. No claim in this paper depends on 32 being optimal.

We did not run a controlled ablation comparing the same descriptors extracted from the PCA residual against the same descriptors extracted from the raw image. The pipeline is motivated by the residual step, but this paper does not itself provide the single-variable comparison that would demonstrate the residual step's individual contribution. That comparison is planned as immediate future work.

No robustness test was performed. The numbers describe performance on clean images from the same acquisition and generation pipelines used at training time. They say nothing about performance under JPEG compression, blurring, additive noise, or resizing.

We did not fine-tune a deep baseline such as Xception on this exact split. The 92 to 96 percent figures from [4] and the results from [3] and [9] are cited as context, not as controlled comparisons, since they come from different datasets.

The accuracy and AUC values in Table 1 are point estimates from a single test split. We do not report confidence intervals and note that we lack the statistical evidence to state how robust the ranking is under resampling.

The reproducibility claim covers the data split and the per-image PCA computation, both fixed by the seed. It does not extend to the classifiers' internal randomness, such as RandomForest's bootstrap resampling, which was left at default.
